...
export function ReadPage() {
  const [selectedSection, setSelectedSection] = useState<string>('part-III');
  const [expandedCategory, setExpandedCategory] = useState<'parts' | 'schedules'>('parts');
  const colors = useIndiaColors();
  const textColor = colors.name === 'white' ? '#000080' : colors.primary;

  const toggleCategory = (category: 'parts' | 'schedules') => {
